CREATE OR REPLACE PACKAGE VideoAnalysePackage IS
    ...
    PROCEDURE GetCommentData(par_page_id in nvarchar2, comment_data_cur out sys_refcursor);
...
CREATE OR REPLACE PACKAGE BODY VideoAnalysePackage IS                -----Start of package body
    ...
    PROCEDURE GetCommentData(par_page_id in nvarchar2, comment_data_cur out sys_refcursor) IS
        creation_date timestamp;
        difference_in_hours int;
        difference_in_days int;
    BEGIN
        ...
        IF (difference_in_days > 5 * 7) THEN
            ----------------------------------------------------------
            OPEN comment_data_cur FOR SELECT
                par_page_id "VideoPageId",
                h.week||' week of '||TO_CHAR(h.c_time,'IYYY')||' year' "Label",
                (SELECT COUNT(*)
                    FROM VideoPage vp INNER JOIN "Comment" cm ON vp.Id = cm.VideoPageId
                    WHERE vp.Id = par_page_id AND h.week = TO_CHAR(cm.CommentDate,'IW')) "CommentCount"
        ...
        ELSIF (difference_in_hours > 5 * 24) THEN
            ...
            OPEN comment_data_cur FOR SELECT
                ...
                (SELECT COUNT(*)
                    FROM VideoPage vp INNER JOIN "Comment" cm ON vp.Id = cm.VideoPageId
                    WHERE vp.Id = par_page_id AND h.c_time = TRUNC(cm.CommentDate,'DD')) "CommentCount"
        ...
        ELSE
            ...
            OPEN comment_data_cur FOR SELECT
                ...
                (SELECT COUNT(*)
                    FROM VideoPage vp INNER JOIN "Comment" cm ON vp.Id = cm.VideoPageId
                    WHERE vp.Id = par_page_id AND h.c_time = TRUNC(cm.CommentDate,'HH')) "CommentCount"
